...
###  Цели
 ...
 + Часть 4. Использовать EIGRP named-mode для настройки сети.
...
Включим EIGRP на маршуртизаторах R18 и R17. Аналогично сделаем на остальных устройствах. (тут ошибка именования инстансов eigrp, дальше исправил)

![](./imgs/1.png)

Проверим соседство на роутерах.

![](./imgs/2.png)

Настроим маршруты по умолчанию на R18 и сделаем редистрибьюцию статических маршрутов.

![](./imgs/3.png)

На R16 создаим префикс лист DEFAULT_ONLY, который разрешает только 0.0.0.0/0 маршрут.
Выполним distribute-list prefix DEFAULT_ONLY out Ethernet0/3 ,чтобы отдавать соседу только маршрут по умолчанию.
Проверим на R32, что получаем только 0.0.0.0 маршрут.

![](./imgs/4.png)

Выполним суммаризацию маршрутов 172.16.0.0 на R17 и R16.

![](./imgs/5.png)

Проверим таблицу маршрутизации на л3 свичах.

![](./imgs/6.png)

Пропингуем R32 со свичей 

![](./imgs/7.png)

Иногда, трафик от свичей л3 мог идти через R18. Эт не ок и по этой причине добавим линк между R16 и R17.
Сравним таблицы traceroute и увидим, что в них перестал фигурировать R18 (2-ой хоп).
